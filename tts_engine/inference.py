...
def generate_tokens_from_api(prompt: str, voice: str = DEFAULT_VOICE, temperature: float = TEMPERATURE, 
    ...
    """
    Generate tokens using the Orpheus API with complete streaming support.
    
    This function handles the text-to-speech token generation by interfacing with the API,
    processing the stream response, and yielding individual tokens.
    
    Args:
        prompt (str): The text prompt to generate speech for
    ...
    Yields:
        str: Individual tokens generated by the model
    """
    ...
    if not prompt or not prompt.strip():
        print("[DEBUG TOKEN] Empty prompt provided, returning")
        return
    
    if not API_URL:
        print("[DEBUG TOKEN] No API URL configured")
        return
    ...
    # Thread-local session management for better performance
    session = requests.Session()
    
    # Optimized adapter configuration with improved connection handling
    ...
    # Create a threading event to handle graceful shutdown
    import threading
    ...
    try:
        ...
        print(f"[DEBUG TOKEN] Making API request to {API_URL}")
        ...
        try:
            ...
            if response.status_code != 200:
                print(f"[DEBUG TOKEN] API request failed with status {response.status_code}")
                print(f"Error details: {response.text}")
                stop_event.set()
                return
            
            print(f"[DEBUG TOKEN] API responded with status {response.status_code}, processing stream...")
            ...
            for chunk in response.raw.read_chunked(8192):
                ...
                for line in lines:
                    ...
                    if line_str.startswith('data: '):
                        ...
                        if data_str == '[DONE]':
                            print(f"[DEBUG TOKEN] Stream complete signal received after {token_count} tokens")
                            break
                        
                        try:
                            data = json.loads(data_str)
                            
                            # Extract the token from llama-cpp completions response format
                            if 'choices' in data and len(data['choices']) > 0:
                                choice = data['choices'][0]
                                # For llama-cpp completions, the token is in 'text' field
                                if 'text' in choice:
                                    token = choice['text']
                                    if token:
                            ...
                            if token_count <= 5:  # Only log first few parsing errors
                                print(f"[DEBUG TOKEN] JSON decode error: {e} for line: {data_str[:100]}...")
                            continue
                        except Exception as e:
                            print(f"[DEBUG TOKEN] Error processing token: {e}")
                            continue
                
                if stop_event.is_set():
                    break
            
            print(f"[DEBUG TOKEN] Token generation completed. Total tokens: {token_count}")
            
        except requests.exceptions.RequestException as e:
            print(f"[DEBUG TOKEN] Request failed: {e}")
            return
        except Exception as e:
            print(f"[DEBUG TOKEN] Unexpected error: {e}")
            return
...
async def stream_speech_from_api(
    ...
    print(f"[DEBUG TTS] Starting stream for prompt: '{prompt[:50]}...' with voice: {voice}")
    ...
    async for chunk in tokens_decoder(token_gen):
        chunk_count += 1
        if chunk_count <= 5:  # Only log first few chunks per prompt
            print(f"[DEBUG TTS] Yielding audio chunk {chunk_count} for prompt '{prompt[:20]}...': {len(chunk) if chunk else 0} bytes")
        yield chunk
    
    print(f"[DEBUG TTS] Stream complete for prompt '{prompt[:20]}...': yielded {chunk_count} chunks")

def split_text_into_sentences(text):
    """Split text into sentences with a more reliable approach."""
    print(f"[DEBUG SPLIT] Input text ({len(text)} chars): '{text[:100]}{'...' if len(text) > 100 else ''}'")
    ...
    print(f"[DEBUG SPLIT] Initial split into {len(parts)} parts")
    for i, part in enumerate(parts[:5]):  # Only log first 5
        print(f"[DEBUG SPLIT] Part {i}: '{part[:60]}{'...' if len(part) > 60 else ''}'")
    ...
    print(f"[DEBUG SPLIT] Final combined into {len(combined_sentences)} sentences")
    for i, sentence in enumerate(combined_sentences[:3]):  # Only log first 3
        print(f"[DEBUG SPLIT] Final sentence {i}: '{sentence[:60]}{'...' if len(sentence) > 60 else ''}'")
